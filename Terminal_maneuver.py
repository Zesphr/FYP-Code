...
def trajectory(alpha, plot_ops, ti, tf, thrust, vals):

    def ground(t, states, alpha, thrust):
        return states[3]
    ...
    sol1 = solve_ivp(ode, [ti, tf], y0=vals, t_eval=time, method='RK45',
                     args=(alpha, thrust), atol=1e-7, rtol=1e-4, events=ground, max_step=0.5)
    ...
    if plot_ops[0] is True:
        ...
        ##### DO NOT PLOT FIGURE 6, IT IS RESERVED FOR COST FUNCTION
        ...
        plt.title('Trajectory Profile - Unconstrained')
        ...
        plt.xlabel('Time (t)')
        plt.ylabel('Altitude (m)')
        plt.title('Vertical Profile - Unconstrained')
        ...
        plt.title('Velocity Variation - Unconstrained')
        ...
        plt.xlabel('Time (t)')
        plt.ylabel('Mass (kg)')
        plt.title('Mass Loss Due to Fuel - Unconstrained')
        ...
        plt.title('Angle of Attack Profile - Unconstrained')
...
def single_shooting(param, plot=False, colour='b', style='-'):
    ...
    while True:
        ...
        vals_f = trajectory(aoa, plot_ops, t_int, t_final, thrust, vals_i)
...
guess = np.array([4] * 0 + [3] * 9)
bounds = [(-10, 40)] * 0 + [(-8, 9)] * 9
...
plt.ylabel('Range (m)')
plt.title('Range per Function Evaluation - Unconstrained')
...
plt.xlabel('Iteration')
plt.ylabel('Cost')
plt.title('SLSQP Cost per Function Evaluation - Unconstrained')
plt.show()
...
for i in range(0,5):
    ...
    xopt, fopt = pso(objective_function, lb, ub, args=(False, 'r', ':',), swarmsize=400, maxiter=50, debug=True)
    ...
    model = ga(function=objective_function, dimension=9, variable_type='real', variable_boundaries=bnds, algorithm_parameters=algorithm_param,)
    result = model.run()
